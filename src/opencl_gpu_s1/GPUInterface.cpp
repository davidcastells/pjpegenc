 ...
 * Author: dcr
 * 
 * Created on January 30, 2020, 6:39 PM
 */
...
{
    GPUInterface* pObj = (GPUInterface*) user_data;
    pObj->notifyBuild(prog);
}

void GPUInterface::notifyBuild(cl_program program)
{
    cl_int ret;
    size_t logSize;
    char* sLog;
...
void GPUInterface::initOpenCL()
{
    cl_uint spi = 0;
    cl_uint sdi = 0;

    if (m_verbose)
        cout << "[OCLGPU] initialization (version compiled "  << __DATE__  << " "  << __TIME__ << ")" << endl;
    ...
    try
    {
        ...
        {
            if (m_verbose)
                cout << "[OCLGPU] ### ERROR ### Failed to change dir" << endl;
            return;
        }
...
//        initKernels(ct, pi, di, cq, sizeof(unsigned int), NUM_XMP_BITS / 32, NUM_XMP_COUNT, openCLFilesPath);
...
//
//void GPUInterface::initKernels(cl_context context, 
...
//        
//	cl_int ret;
//	m_memA = clCreateBuffer(context, CL_MEM_READ_ONLY, dataSize*numLimbs*numCount, NULL, &ret);
//	SAMPLE_CHECK_ERRORS(ret);
//	m_memB = clCreateBuffer(context, CL_MEM_READ_ONLY, dataSize*numLimbs*numCount, NULL, &ret);
//	SAMPLE_CHECK_ERRORS(ret);
//        m_memC = clCreateBuffer(context, CL_MEM_READ_ONLY, dataSize*numLimbs*numCount, NULL, &ret);
//	SAMPLE_CHECK_ERRORS(ret);
//        m_memD = clCreateBuffer(context, CL_MEM_READ_ONLY, dataSize*numLimbs*numCount, NULL, &ret);
//	SAMPLE_CHECK_ERRORS(ret);
//        m_memE = clCreateBuffer(context, CL_MEM_READ_ONLY, dataSize*numLimbs*numCount, NULL, &ret);
//	SAMPLE_CHECK_ERRORS(ret);
//	m_memQ = clCreateBuffer(context, CL_MEM_WRITE_ONLY, dataSize*numLimbs*numCount, NULL, &ret);
//	SAMPLE_CHECK_ERRORS(ret);
//	m_memR = clCreateBuffer(context, CL_MEM_WRITE_ONLY, dataSize*numLimbs*numCount, NULL, &ret);
//	SAMPLE_CHECK_ERRORS(ret);
//
//        if (verbose)
//        {
...
//        
//        printf("[OCLFPGAJNI] Platform Name = %s\n", platformName.c_str());
//
//        cl_program program;
//                
...
//            
//#ifdef USE_KERNEL_MULT_MOD_APUINT_ND_UNSAFE
//            fullPath = openCLFilesPath.c_str() + std::string("/multMod_apuint_nd_unsafe.aocx");
//#endif
//            
//#ifdef USE_KERNEL_MULT_MOD_APUINT_PI_UNSAFE
//            fullPath = openCLFilesPath.c_str() + std::string("/" KERNEL_FILE);
//#endif
//            
//#ifdef USE_KERNEL_MULT_MOD_APUINT_MHW_ND
//            fullPath = openCLFilesPath.c_str() + std::string("/" KERNEL_FILE);
//#endif
//            
//#ifdef USE_KERNEL_POW_MOD_INTERLEAVED_BASE_PI_SAFE
//            fullPath = openCLFilesPath.c_str() + std::string("/powMod_interleaved_base_pi_safe.aocx");
//#endif            
//            
//#ifdef USE_KERNEL_POW_MOD_APUINT_MHW_ND
//            fullPath = openCLFilesPath.c_str() + std::string("/powMod_apuint_mhw_nd.aocx");
//#endif
//            
//#ifdef USE_KERNEL_POW_MOD_APUINT_MHW_PI
//            fullPath = openCLFilesPath.c_str() + std::string("/powMod_apuint_mhw_pi.aocx");
//#endif
//
//#ifdef USE_KERNEL_POW_MOD_FLEX_MMP_PI
//            fullPath = openCLFilesPath.c_str() + std::string("/") + m_kernelFile;
//#endif
//
